  ...
  <main class="modal" v-if="isOpen">
    ...
    <BaseCard class="modal__content">
      <header class="modal__content__header">
        <slot name="header">
          <span class="text-txt-color ml-6 mr-6">{{ title }}</span>
          <svg v-if="closeButton" @click="emit('close')" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="20" cy="20" r="20" fill="#F7F7F7" />
            <path d="M26 14L14 26M14 14L26 26" stroke="#141414" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </slot>
        <Icon name="mdi:close" class="close-button cursor-pointer text-gray-400" size="25" @click="emit('close')" />
      </header>
      <!-- Body -->
      <slot></slot>
    </BaseCard>
